...
author:
  ...
  - name: Filippo Gambarota
    ...
    affiliations:
      - name: University of Padua
        department: Department of Developemntal Psychology and Socialization
        address: 8 Via Venezia
        city: Padua
        region: PD
        country: Italy
...
In the next sections, we will repeat and deepen the multiple causes of distorsion of NCA estimates with psychological data.
...
These concerns are particularly important in psychology because nonnormality is not an exotic edge case [@toffaliniClustersThatAre2024; @cainUnivariateMultivariateSkewness2017]. Although NCA does not assumes normality, the kinds of distributional asymmetries that are routine in psychological data—but are not properties of the constructs—are exactly the kinds of features that can create or exaggerate the sparse-corner patterns on which NCA relies [@sorjonenNecessityFunctionSkewness2017; @sorjonenPredictingSignificanceNecessity2019].
...
This decomposition makes it possible to separate changes due to sample selection from additional changes introduced by measurement and scoring after selection. However, this manipulation also affects sample size, which may also affect the results compared to the other manipalutions.
